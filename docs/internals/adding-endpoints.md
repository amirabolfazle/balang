...
This is mostly useful if you've managed to catch a new Bale Bot API update
...
What type should `ChatID` be? Bale allows specifying numeric chat IDs or
...
Note that `ChatID` is an `int64`. Bale chat IDs can be greater than 32 bits.
...
`map[string]string`. Bale expects only a single field for `chat_id`, so we
...
Bale only requires two fields to call `deleteMessage`, so this will be fast.
